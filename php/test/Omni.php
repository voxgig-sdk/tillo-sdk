...
//    only - a typed TilloContext there would make every ctx
...
final class TilloOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TilloContext($clean, null);
    ...
    public static function err_from_map($m): ?TilloError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TilloError(is_string($code) ? $code : '', $msg);
    }
