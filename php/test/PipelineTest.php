...
{
    private static function utility(): TilloUtility
    {
        return new TilloUtility();
    }
    ...
    private static function ctx(array $over = []): TilloContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TilloContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TilloOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TilloError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TilloOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TilloMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TilloOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TilloMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TilloOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TilloMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TilloSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TilloMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TilloResponse([]),
            'result' => new TilloResult([]),
        ]);
        [, $err] = TilloMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TilloSpec(['step' => 's']),
            'response' => new TilloResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TilloResult([]),
        ]);
        [, $err] = TilloMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TilloSpec(['step' => 's']),
            'response' => new TilloResponse(self::resp(200, ['v' => 1])),
            'result' => new TilloResult([]),
        ]);
        [, $err] = TilloMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TilloSpec(['step' => 's']),
            'response' => new TilloResponse(self::resp(200, ['v' => 2])),
            'result' => new TilloResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TilloMakeResponse::call($ctx);
    ...
    {
        $preset = new TilloResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TilloSpec([]),
            'response' => new TilloResponse([]),
            'result' => new TilloResult([]),
        ...
        [$got, $err] = TilloMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TilloResult([])]);
        [, $err] = TilloMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TilloSpec([]), 'result' => null]);
        [, $err] = TilloMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TilloSpec(['step' => 's']),
            'result' => new TilloResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TilloOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TilloMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TilloSpec(['step' => 's']),
            'result' => new TilloResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TilloOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TilloMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TilloResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TilloSpec([]),
            'result' => new TilloResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TilloMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new TilloError('boom', 'boom');
        $utility->fetcher = function (TilloContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TilloSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TilloMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TilloContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TilloMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TilloResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TilloContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TilloSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TilloMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TilloResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TilloSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TilloMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TilloMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TilloMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TilloResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TilloDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TilloResult(['ok' => false])]);
        $this->expectException(TilloError::class);
        TilloDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TilloResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TilloMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TilloBaseFeature();
        $b = new TilloBaseFeature();
        TilloFeatureAdd::call($ctx, $a);
        TilloFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TilloBaseFeature
    {
        $f = new TilloBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): TilloContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TilloSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new TilloResponse(['headers' => null]),
            'result' => new TilloResult([]),
        ]);
        TilloResultHeaders::call($ctx);
