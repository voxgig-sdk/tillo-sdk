...
class FloatEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TilloSDK::test(null, null);
        $ent = $testsdk->Float(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Float(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Float(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set TILLO_TEST_FLOAT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function float_basic_setup($extra)
{
    Runner::load_env_local();

    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/float/FloatTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["float01", "float02", "float03"] as $k) {
    ...
    $entid_env_raw = getenv("TILLO_TEST_FLOAT_ENTID");
