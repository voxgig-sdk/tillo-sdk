...
class FtCtrl extends TilloControl
{
...
class FtClient
{
    ...
    private TilloUtility $utility;

    public function __construct(TilloUtility $utility, array $options)
    {
    ...
    public function get_utility(): TilloUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TilloContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TilloContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TilloConfig::make_feature($name);
    ...
    private static function buildUrl(TilloSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TilloError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TilloResponse($fetched);
        }
    ...
    private function fail(TilloContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TilloContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
