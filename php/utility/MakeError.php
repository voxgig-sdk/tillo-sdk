...
class TilloMakeError
{
    public static function call(?TilloContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TilloContext([], null);
        }
        $op = $ctx->op ?? new TilloOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TilloResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TilloError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TilloError('', $msg, $ctx);
