...
func (sdk *TilloSDK) OptionsMap() map[string]any {
...
func (sdk *TilloSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *TilloSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *TilloSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *TilloSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *TilloSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *TilloSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("TilloSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *TilloSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *TilloSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("TilloSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func (sdk *TilloSDK) Brand(data map[string]any) TilloEntity {
...
// Float returns a Float entity bound to this client.
// Idiomatic usage: client.Float(nil).List(nil, nil) or
