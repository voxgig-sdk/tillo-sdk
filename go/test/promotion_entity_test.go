...
func TestPromotionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set TILLO_TEST_PROMOTION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if promotionRef01DataDt0Loaded == nil {
...
func promotionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TILLO_TEST_PROMOTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["TILLO_TEST_PROMOTION_ENTID"])
