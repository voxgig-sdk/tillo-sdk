...
import (
	...
	omni "github.com/voxgig-sdk/tillo-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TilloError); is {
				out := map[string]any{
					"name":    "TilloError",
...
func omniCtx(arg any, client *sdk.TilloSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TilloSDK); is {
			client = live
