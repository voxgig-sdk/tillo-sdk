// Command Tillo-mcp is an MCP server exposing the Tillo
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Tillo_list  — list records of an entity
//	Tillo_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: TILLO_APIKEY carries the API key and
	// TILLO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TILLO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TILLO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTilloSDK(opts)
