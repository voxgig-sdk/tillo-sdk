...
[MCP](https://modelcontextprotocol.io) server exposing the Tillo SDK as
two agent tools — `tillo_list` and `tillo_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tillo-mcp)
make build
...
claude mcp add --scope user tillo \
  -- /absolute/path/to/tillo-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./tillo-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tillo-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user tillo \
     -- "$PWD"/dist/*/tillo-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `tillo_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `tillo_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TILLO_APIKEY` | API key sent with every request. |
| `TILLO_BASE` | Optional override of the API base URL. |
...
```sh
./tillo-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"tillo_load","arguments":{"entity":"brand","query":{"id":1}}}}'
```
