...
boru-driven command-line client **and** interactive REPL for the Tillo
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/tillo-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/tillo-cli
   ```
...
Run with no arguments to open a REPL (prompt `tillo>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/tillo-cli`. |
...
(`./tillo-cli <expr>`) and interactively (the REPL), and expressions compose the
