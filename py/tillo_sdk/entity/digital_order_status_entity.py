...
class DigitalOrderStatusEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DigitalOrderStatusEntity(self._client, opts)
    ...
    def data_get(self) -> DigitalOrderStatus:
    ...
    def match_get(self) -> DigitalOrderStatus:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DigitalOrderStatus:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DigitalOrderStatus().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
