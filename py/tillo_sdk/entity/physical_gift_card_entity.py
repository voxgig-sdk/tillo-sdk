...
class PhysicalGiftCardEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PhysicalGiftCardEntity(self._client, opts)
    ...
    def data_get(self) -> PhysicalGiftCard:
    ...
    def match_get(self) -> PhysicalGiftCard:
    ...
    def create(self, reqdata: PhysicalGiftCardCreateData, ctrl=None) -> PhysicalGiftCard:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PhysicalGiftCard:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PhysicalGiftCard().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
