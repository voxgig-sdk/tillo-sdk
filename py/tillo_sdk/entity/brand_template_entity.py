...
class BrandTemplateEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BrandTemplateEntity(self._client, opts)
    ...
    def data_get(self) -> BrandTemplate:
    ...
    def match_get(self) -> BrandTemplate:
    ...
    def load(self, reqmatch=None, ctrl=None) -> BrandTemplate:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.BrandTemplate().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
