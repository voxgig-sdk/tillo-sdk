...
class DigitalIssueDeleteEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DigitalIssueDeleteEntity(self._client, opts)
    ...
    def data_get(self) -> DigitalIssueDelete:
    ...
    def match_get(self) -> DigitalIssueDelete:
    ...
    def create(self, reqdata: DigitalIssueDeleteCreateData, ctrl=None) -> DigitalIssueDelete:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> DigitalIssueDelete:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.DigitalIssueDelete().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
