...
function isRunnable(code) {
    return (/new\s+TilloSDK\b/.test(code) ||
        /\bTilloSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+TilloSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/TilloSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
