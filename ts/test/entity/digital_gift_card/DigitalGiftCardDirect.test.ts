...
describe('DigitalGiftCardDirect', async () => {
  ...
  test('direct-load-digital_gift_card', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-digital_gift_card', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TILLO_TEST_DIGITAL_GIFT_CARD_ENTID']
