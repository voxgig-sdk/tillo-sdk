...
describe('DigitalGiftCardEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'digital_gift_card.' + op, live)) return
    }
    ...
    assert(null != digital_gift_card_ref01_data)


    // LOAD
    const digital_gift_card_ref01_match_dt0: any = {}
    const digital_gift_card_ref01_data_dt0 = (await digital_gift_card_ref01_ent.load(digital_gift_card_ref01_match_dt0)).data()
    assert(null != digital_gift_card_ref01_data_dt0)
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/digital_gift_card/DigitalGiftCardTestData.json')
