...
describe('BrandTemplateEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'brand_template.' + op, live)) return
    }
    ...
    let brand_template_ref01_data = Object.values(setup.data.existing.brand_template)[0] as any

    // LOAD
    const brand_template_ref01_ent = client.BrandTemplate()
    const brand_template_ref01_match_dt0: any = {}
