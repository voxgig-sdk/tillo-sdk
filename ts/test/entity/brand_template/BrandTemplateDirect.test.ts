...
describe('BrandTemplateDirect', async () => {
  ...
  test('direct-load-brand_template', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-brand_template', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TILLO_TEST_BRAND_TEMPLATE_ENTID']
