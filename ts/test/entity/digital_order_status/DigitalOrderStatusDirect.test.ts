...
describe('DigitalOrderStatusDirect', async () => {
  ...
  test('direct-load-digital_order_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-digital_order_status', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['TILLO_TEST_DIGITAL_ORDER_STATUS_ENTID']
