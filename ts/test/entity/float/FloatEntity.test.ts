...
describe('FloatEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'float.' + op, live)) return
    }
    ...
    let float_ref01_data = Object.values(setup.data.existing.float)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['TILLO_TEST_FLOAT_ENTID']
