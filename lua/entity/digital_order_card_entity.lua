-- Tillo SDK DigitalOrderCard entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DigitalOrderCardEntity = {}
DigitalOrderCardEntity.__index = DigitalOrderCardEntity


function DigitalOrderCardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DigitalOrderCardEntity)
  self._name = "digital_order_card"
  self._client = client
...
function DigitalOrderCardEntity:get_name()
  return self._name
end


function DigitalOrderCardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DigitalOrderCardEntity.new(self._client, opts)
end
...
function DigitalOrderCardEntity:mark_deleted()
  self._deleted = true
end


function DigitalOrderCardEntity:deleted()
  return true == self._deleted
end


function DigitalOrderCardEntity:data_set(args)
  if args ~= nil then
...
function DigitalOrderCardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DigitalOrderCardEntity:match_set(args)
  if args ~= nil then
...
function DigitalOrderCardEntity:match_get()
...
function DigitalOrderCardEntity:stream(action, args, callopts)
...
---@param reqdata DigitalOrderCardCreateData
---@param ctrl? table
---@return DigitalOrderCard
---@return string? err
function DigitalOrderCardEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DigitalOrderCardEntity:_run_op(ctx, post_done)
  local utility = self._utility
