...
local DgcEntity = {}
DgcEntity.__index = DgcEntity


function DgcEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DgcEntity)
  self._name = "dgc"
  self._client = client
...
function DgcEntity:get_name()
  return self._name
end


function DgcEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DgcEntity.new(self._client, opts)
end
...
function DgcEntity:mark_deleted()
  self._deleted = true
end


function DgcEntity:deleted()
  return true == self._deleted
end


function DgcEntity:data_set(args)
  if args ~= nil then
...
function DgcEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DgcEntity:match_set(args)
  if args ~= nil then
...
function DgcEntity:stream(action, args, callopts)
...
---@param reqdata DgcCreateData
---@param ctrl? table
---@return Dgc
---@return string? err
function DgcEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function DgcEntity:_run_op(ctx, post_done)
  local utility = self._utility
