-- Tillo SDK DigitalOrderStatus entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DigitalOrderStatusEntity = {}
DigitalOrderStatusEntity.__index = DigitalOrderStatusEntity


function DigitalOrderStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DigitalOrderStatusEntity)
  self._name = "digital_order_status"
  self._client = client
...
function DigitalOrderStatusEntity:get_name()
  return self._name
end


function DigitalOrderStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DigitalOrderStatusEntity.new(self._client, opts)
end
...
function DigitalOrderStatusEntity:mark_deleted()
  self._deleted = true
end


function DigitalOrderStatusEntity:deleted()
  return true == self._deleted
end


function DigitalOrderStatusEntity:data_set(args)
  if args ~= nil then
...
function DigitalOrderStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DigitalOrderStatusEntity:match_set(args)
  if args ~= nil then
...
function DigitalOrderStatusEntity:match_get()
...
function DigitalOrderStatusEntity:stream(action, args, callopts)
...
---@param reqmatch DigitalOrderStatusLoadMatch
---@param ctrl? table
---@return DigitalOrderStatus
---@return string? err
function DigitalOrderStatusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DigitalOrderStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return DigitalOrderStatusEntity
