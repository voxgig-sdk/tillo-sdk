-- Tillo SDK Brand entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local BrandEntity = {}
BrandEntity.__index = BrandEntity


function BrandEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BrandEntity)
  self._name = "brand"
  self._client = client
...
function BrandEntity:get_name()
  return self._name
end


function BrandEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BrandEntity.new(self._client, opts)
end
...
function BrandEntity:mark_deleted()
  self._deleted = true
end


function BrandEntity:deleted()
  return true == self._deleted
end


function BrandEntity:data_set(args)
  if args ~= nil then
...
function BrandEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BrandEntity:match_set(args)
  if args ~= nil then
...
function BrandEntity:match_get()
...
function BrandEntity:stream(action, args, callopts)
...
---@param reqmatch BrandLoadMatch
---@param ctrl? table
---@return Brand
---@return string? err
function BrandEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BrandEntity:_run_op(ctx, post_done)
  local utility = self._utility
