-- Tillo SDK PhysicalOrderStatus entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PhysicalOrderStatusEntity = {}
PhysicalOrderStatusEntity.__index = PhysicalOrderStatusEntity


function PhysicalOrderStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PhysicalOrderStatusEntity)
  self._name = "physical_order_status"
  self._client = client
...
function PhysicalOrderStatusEntity:get_name()
  return self._name
end


function PhysicalOrderStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PhysicalOrderStatusEntity.new(self._client, opts)
end
...
function PhysicalOrderStatusEntity:mark_deleted()
  self._deleted = true
end


function PhysicalOrderStatusEntity:deleted()
  return true == self._deleted
end


function PhysicalOrderStatusEntity:data_set(args)
  if args ~= nil then
...
function PhysicalOrderStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PhysicalOrderStatusEntity:match_set(args)
  if args ~= nil then
...
function PhysicalOrderStatusEntity:match_get()
...
function PhysicalOrderStatusEntity:stream(action, args, callopts)
...
---@param reqdata PhysicalOrderStatusCreateData
---@param ctrl? table
---@return PhysicalOrderStatus
---@return string? err
function PhysicalOrderStatusEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function PhysicalOrderStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return PhysicalOrderStatusEntity
