...
local FloatEntity = {}
FloatEntity.__index = FloatEntity


function FloatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FloatEntity)
  self._name = "float"
  self._client = client
...
function FloatEntity:get_name()
  return self._name
end


function FloatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FloatEntity.new(self._client, opts)
end
...
function FloatEntity:mark_deleted()
  self._deleted = true
end


function FloatEntity:deleted()
  return true == self._deleted
end


function FloatEntity:data_set(args)
  if args ~= nil then
...
function FloatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FloatEntity:match_set(args)
  if args ~= nil then
...
function FloatEntity:match_get()
...
function FloatEntity:stream(action, args, callopts)
...
---@return Float[]
---@return string? err
function FloatEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function FloatEntity:_run_op(ctx, post_done)
  local utility = self._utility
