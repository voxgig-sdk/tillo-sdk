-- Tillo SDK Promotion entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PromotionEntity = {}
PromotionEntity.__index = PromotionEntity


function PromotionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PromotionEntity)
  self._name = "promotion"
  self._client = client
...
function PromotionEntity:get_name()
  return self._name
end


function PromotionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PromotionEntity.new(self._client, opts)
end
...
function PromotionEntity:mark_deleted()
  self._deleted = true
end


function PromotionEntity:deleted()
  return true == self._deleted
end


function PromotionEntity:data_set(args)
  if args ~= nil then
...
function PromotionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PromotionEntity:match_set(args)
  if args ~= nil then
...
function PromotionEntity:match_get()
...
function PromotionEntity:stream(action, args, callopts)
...
---@param reqmatch PromotionLoadMatch
---@param ctrl? table
---@return Promotion
---@return string? err
function PromotionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PromotionEntity:_run_op(ctx, post_done)
  local utility = self._utility
