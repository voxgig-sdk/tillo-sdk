...
TilloSDK._make_feature = _make_feature


function TilloSDK.new(options)
  local self = setmetatable({}, TilloSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function TilloSDK:get_utility()
  return Utility.copy(self._utility)
end


function TilloSDK:get_root_ctx()
  return self._rootctx
end


function TilloSDK:prepare(fetchargs)
...
function TilloSDK:direct(fetchargs)
...
-- by passing it.
function TilloSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function TilloSDK:graphql(query, variables, ctrl)
...
-- Idiomatic facade: client:Float():list() / client:Float():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function TilloSDK:Float(data)
  local EntityMod = require("entity.float_entity")
  if data == nil then
    if self._float == nil then
...
function TilloSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = TilloSDK.new(sdkopts)
  sdk.mode = "test"
