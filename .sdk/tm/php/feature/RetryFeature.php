...
class TilloRetryFeature extends TilloBaseFeature
{
    ...
    public function init(TilloContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TilloContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TilloContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TilloContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
